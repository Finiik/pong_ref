...
- **Об’єктно-орієнтований підхід:**  
  Кожна гра реалізована як клас `Game`, що об'єднує логіку, стан і поведінку. Це робить код гнучким, легко масштабованим і зручним для тестування.

- **Розділення логіки за методами:**  
  Код поділений на функції: `handle_input`, `update_game`, `draw_game`, `reset_game`, що зменшує складність кожного блоку та спрощує підтримку.

- **Використання констант:**  
  Усі магічні числа винесені в константи (`CELL_SIZE`, `WIDTH`, `PLAYER_WIDTH`, `FPS` тощо) для зручної зміни параметрів гри.

- **Уніфікований інтерфейс (UI/UX):**  
  Рахунок, найкращий результат, повідомлення про паузу, поразку та інструкції до керування мають єдиний стиль у кожній грі.

- **Керування станами гри:**  
  Введено логіку `Paused`, `Game Over`, `Restart`, `ESC`, що значно підвищує користувацький досвід і дозволяє керувати грою без перезапуску.

- **Збереження результатів (БД):**  
  Через `Models.User` та `db_config` зберігаються найкращі результати користувача, що додає глибину до гри та мотивацію до покращення.

- **Аудіо підтримка (Space Invader):**  
  Підтримка звукових ефектів/музики в грі та меню (`menu_music`, `game_music`) дозволяє створити атмосферу гри.

- **Відокремлення логіки та рендерингу:**  
  Логіка гри (`update_game`) та її візуалізація (`draw_game`) розділені, що дозволяє при потребі змінювати візуальну частину без впливу на механіку.

- **Центрування елементів:**  
  Елементи інтерфейсу та поля вирівнюються відносно центру/ігрової зони, що робить вигляд адаптивним до змін розміру вікна.

- **Підготовка до масштабування:**  
  Код легко адаптувати до нових рівнів, ворогів, power-up’ів, AI, мультиплеєра тощо — усе вже структуровано під розширення.
...
| Параметр                             | Pacman (до)   |Pacman (після)|Space Invader (до) |Space Invader (після) |
|--------------------------------------|---------------|--------------|-------------------|----------------------|
| **Стиль**                            | Процедурний   | ООП          | Процедурний       | ООП                  |
| **Клас `Game`**                      | Немає         | Є            | Немає             | Є                    |
| **Константи**                        | В коді        | Є            | Частково          | Є                    |
| **Обробка подій**                    | У `while`     | Через методи | У `while`         | Через методи         |
| **Пауза / Рестарт / ESC**            | Немає         | Є            | Немає             | Є                    |
| **UI рахунок, інструкції**           | Відсутні      | Є            | Відсутні          | Є                    |
| **Масштабованість**                  | Низька        | Висока       | Низька            | Висока               |
| **Збереження прогресу**              | Немає         | Є            | Немає             | Є                    |
| **Музика**                           | Немає         |Немає         | Немає             | Є                    |
| **Коментарі, структура**             | Відсутні      | Чітка        | Мінімальні        | Чітка                |
| **Візуальне вирівнювання елементів** | Немає         | Є            | Немає             | Є                    |
